...
-- Allow group members to see each other's rounds
create policy "Group members can view each other's rounds"
  on rounds for select using (
    exists (
      select 1 from group_members gm1
      join group_members gm2 on gm1.group_id = gm2.group_id
      where gm1.user_id = auth.uid()
        and gm2.user_id = rounds.user_id
    )
  );
...
-- Anyone can look up a group by invite code (for joining)
